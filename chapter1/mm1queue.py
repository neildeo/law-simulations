...
class Experiment:
    ...
    def __init__(self, arrival_rate: float, service_rate: float, seed=None) -> None:
    ...
    def get_results(self):
        avg_number_in_system = integrate_trace(
        ...
        print("----------------------\n")
        print(
            f"Time-average number of customers in system: {avg_number_in_system / self.env.now:.3f}")
        print(
            f"Average total time in system: {np.array(self.results.lists['total_times']).mean():.3f}")
        print(
            f"Maximum queue length: {self.results.metrics['max_queue_length']}"
        )
        print(
        ...
        plt.step(times, lengths, where='post', lw=.7)
        plt.xlabel('Time')
    ...
    def customer(self):
        ...
        with self.server.request() as req:
            if len(self.server.queue) > self.results.metrics['max_queue_length']:
                self.results.metrics['max_queue_length'] = len(
                    self.server.queue)
            ...
            self.results.lists['delays'].append(
                delay)
            self.results.lists['total_times'].append(
                total_time)
            self.results.traces['number_in_system'].append(
                (self.env.now, self.number_in_system)
            )
